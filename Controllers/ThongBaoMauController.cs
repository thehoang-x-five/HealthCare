...
namespace HealthCare.Controllers
{
    ...
    public class ThongBaoMauController : ControllerBase
    {
        ...
        public async Task<ActionResult<ThongBaoMau>> Create([FromBody] ThongBaoMauCreateRequest request)
        {
            ...
            {
                MaMau = System.Guid.NewGuid().ToString("N"),
                TenMau = request.TenMau,
                NoiDungMau = request.NoiDungMau,
                BienDong = request.BienDong,
                NgayTao = now,
                NgayCapNhat = now,
                KichHoat = request.KichHoat ?? true
            };

            _db.ThongBaoMaus.Add(entity);
            await _db.SaveChangesAsync();

            return CreatedAtAction(nameof(GetById), new { id = entity.MaMau }, entity);
        }
        ...
        public async Task<ActionResult<ThongBaoMau>> Update(string id, [FromBody] ThongBaoMauUpdateRequest request)
        {
            ...
            if (request.BienDong != null)
                entity.BienDong = request.BienDong;

            if (request.KichHoat.HasValue)
                entity.KichHoat = request.KichHoat.Value;

            entity.NgayCapNhat = System.DateTime.Now;
    ...
    // ===== DTOs for ThongBaoMau (matching main's entity fields) =====

    public class ThongBaoMauCreateRequest
    {
        public string TenMau { get; set; } = default!;
        public string NoiDungMau { get; set; } = default!;
        public string? BienDong { get; set; } // JSON array: ["ten_benh_nhan", "gio_hen", ...]
