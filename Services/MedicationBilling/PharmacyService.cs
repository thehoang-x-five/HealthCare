            ...
            try
            {
                    ...
                    !string.IsNullOrWhiteSpace(don.MaBenhNhan))
                {
                    ...
                    foreach (var item in chiTiet)
                    {
                        ...
                        {
                            { "ma_thuoc", item.MaThuoc ?? (BsonValue)BsonNull.Value },
                            { "ten_thuoc", item.KhoThuoc?.TenThuoc ?? (BsonValue)BsonNull.Value },
                            { "hoat_chat", item.KhoThuoc?.CongDung ?? (BsonValue)BsonNull.Value },
                            { "don_vi", item.KhoThuoc?.DonViTinh ?? (BsonValue)BsonNull.Value },
                            { "so_luong", item.SoLuong },
                            { "don_gia", item.KhoThuoc?.GiaNiemYet ?? 0m },
                            { "lieu_dung", item.LieuDung ?? (BsonValue)BsonNull.Value },
                            { "tan_suat_dung", item.TanSuatDung ?? (BsonValue)BsonNull.Value },
                            { "so_ngay_dung", item.SoNgayDung ?? (BsonValue)BsonNull.Value },
                            { "ghi_chu", item.GhiChu ?? (BsonValue)BsonNull.Value }
                        });
                    }
                    ...
                    var payload = new BsonDocument
                    {
                        ...
                        { "ma_bac_si_ke_don", don.MaBacSiKeDon ?? (BsonValue)BsonNull.Value },
                        { "ten_bac_si", don.BacSiKeDon?.HoTen ?? (BsonValue)BsonNull.Value },
